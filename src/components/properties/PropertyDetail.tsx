...
export function PropertyDetail({ property, role, onClose, onEdit, onDelete, visibleColumnNames }: PropertyDetailProps) {
  ...
  if (show('nombre_kibah') && hasValue(property.nombre_kibah)) infoFields.push({ label: 'Nombre Kibah', value: String(property.nombre_kibah) })
  ...
  if (show('colonia') && hasValue(property.colonia)) ubicFields.push({ label: 'Colonia', value: String(property.colonia) })
  if (show('alcaldia') && hasValue(property.alcaldia)) ubicFields.push({ label: 'Alcaldía', value: String(property.alcaldia) })
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-4">
          ...
          {/* Info General */}
          {infoFields.length > 0 && (
            <div className="py-5 border-t" style={{ borderColor: 'rgba(148,163,184,0.15)' }}>
