...
export default function AdminCalendarioPage() {
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex items-center border border-border-primary rounded-full overflow-hidden">
            <button
              ...
              className={`h-8 px-3 text-xs font-medium transition-colors cursor-pointer
                ${viewMode === 'personal' ? 'bg-orange text-white' : 'text-text-secondary hover:text-text-primary'}`}
            >
              Mi calendario
            </button>
            <button
              ...
              className={`h-8 px-3 text-xs font-medium transition-colors cursor-pointer
                ${viewMode === 'admin' ? 'bg-orange text-white' : 'text-text-secondary hover:text-text-primary'}`}
            >
              Global
            </button>
          </div>
      ...
      {viewMode === 'admin' && (
        <AsesorFilter value={filterUserId} onChange={setFilterUserId} />
      )}

      <CalendarView
        mode={viewMode}
        asesorUserId={viewMode === 'admin' ? filterUserId || undefined : undefined}
      />

      {showSelector && (
        <CalendarSelector onSelect={() => setShowSelector(false)} />
      )}
